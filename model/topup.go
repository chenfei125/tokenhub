...
func RechargeAlipay(tradeNo string) (err error) {
	...
	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("支付宝充值成功，充值额度: %v，支付金额: %.2f CNY", logger.FormatQuota(quotaToAdd), topUp.Money))
	}

	return nil
}

// RechargeWechatPay 微信支付充值完成处理
func RechargeWechatPay(tradeNo string) (err error) {
	...
	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("微信支付充值成功，充值额度: %v，支付金额: %.2f CNY", logger.FormatQuota(quotaToAdd), topUp.Money))
	}
