...
export default function MiniMaxApi() {
  ...
  const c = SEO.miniMaxApi[lang] || SEO.miniMaxApi['en'];
  ...
  return (
    <div className='max-w-3xl mx-auto px-4 py-16 space-y-12'>
      ...
      <section>
        <h2 className='text-2xl font-bold text-semi-color-text-0 mb-3'>{c.scenariosTitle}</h2>
        <p className='text-semi-color-text-1 leading-relaxed mb-4'>{c.scenariosIntro}</p>
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
          {c.scenarios.map((item, i) => (
        ...
        <ol className='space-y-3'>
          {c.steps.map((step, i) => (
            <li key={i} className='flex gap-3'>
              ...
              <span className='text-sm text-semi-color-text-1'>{step}</span>
            </li>
          ))}
        </ol>
      </section>
