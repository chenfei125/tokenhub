...
export default function DeepSeekApi() {
  ...
  const c = SEO.deepSeekApi[lang] || SEO.deepSeekApi['en'];
  ...
  return (
    <div className='max-w-3xl mx-auto px-4 py-16 space-y-12'>
      ...
      <section>
        <h2 className='text-2xl font-bold text-semi-color-text-0 mb-3'>{c.useCasesTitle}</h2>
        <p className='text-semi-color-text-1 leading-relaxed mb-4'>{c.useCasesIntro}</p>
      ...
      <section>
        ...
        <p className='text-semi-color-text-1 leading-relaxed mb-3'>{c.why1}</p>
        <p className='text-semi-color-text-1 leading-relaxed'>{c.why2}</p>
      </section>

      <section>
        <h2 className='text-2xl font-bold text-semi-color-text-0 mb-3'>{c.howTitle}</h2>
        <p className='text-semi-color-text-1 leading-relaxed mb-4'>{c.howIntro}</p>
        <div className='rounded-lg border border-semi-color-border bg-semi-color-fill-0 p-4 font-mono text-sm text-semi-color-text-1 space-y-1 mb-4'>
          <p className='text-semi-color-text-2 text-xs mb-2'>// Before (OpenAI)</p>
          <p>{'base_url = "https://api.openai.com/v1"'}</p>
          <p>{'api_key  = "sk-..."'}</p>
          <p className='text-semi-color-text-2 text-xs mt-3 mb-2'>// After (TokenHub)</p>
